...
class AppEventsLoggerImpl {
    ...
    static void setFlushBehavior(FlushBehavior flushBehavior2) {
        synchronized (staticLock) {
            flushBehavior = flushBehavior2;
        }
    }
    ...
    static String getInstallReferrer() {
        InstallReferrerUtil.tryUpdateReferrerInfo(new Callback() {
    ...
    public void logEvent(String str, Double d, Bundle bundle, boolean z, @Nullable UUID uuid) {
        if (str != null && !str.isEmpty()) {
            ...
            if (FetchedAppGateKeepersManager.getGateKeeperForKey(APP_EVENTS_KILLSWITCH, FacebookSdk.getApplicationId(), false)) {
                Logger.log(LoggingBehavior.APP_EVENTS, str2, "KillSwitch is enabled and fail to log app event: %s", str);
                return;
            }
            try {
                AppEvent appEvent = new AppEvent(this.contextName, str, d, bundle, z, ActivityLifecycleTracker.isInBackground(), uuid);
